...
//数据库查询的偏移量
                ...
                private function pre_page() {
                        if ($this->page_id() == 1) { //页数等于1
                                return "<div class=\"page\"><a class=\"pre\" href=".$this->php_self()."?page_id=1".$this->url().">上一页</a>";//增加了class
                        }elseif ($this->page_id() != 1) { //页数不等于1
                                return "<div class=\"page\"><a class=\"pre\" href=".$this->php_self()."?page_id=".($this->page_id()-1).$this->url().">上一页</a>";//增加了class
                        }
                }
 ...
 return $display_page;
                ...
                private function next_page() {
                        if ($this->page_id() < $this->page_num()) { //页数小于总页数
                                return "<a class=\"readmore\" href=".$this->php_self()."?page_id=".($this->page_id()+1).$this->url().">下一页</a></div>";//增加了class
                        }elseif ($this->page_id() == $this->page_num()) { //页数等于总页数
                                return "<a class=\"readmore\" href=".$this->php_self()."?page_id=".$this->page_num().$this->url().">下一页</a></div>";//增加了class
                        }
                }
...
//                        $page_info .= "<a href=".$this->php_self()."?page_id=1".$this->url().">首页</a> ";
                        $page_info= $this->pre_page();    //隐去其他几条后这条有更改，原来是将这些变量连接到一起，现在把这条变量定义为开头，删掉了“.”
